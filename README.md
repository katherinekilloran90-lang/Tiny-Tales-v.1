...
1. **Homepage form** (`app/page.tsx`, `components/StoryForm`) collects: idea, optional child's name, age range, story style, illustration style, and story length. A row of tappable inspiration chips (`components/StoryForm/IdeaSuggestions.tsx`) can fill the idea field with a starter prompt, which stays fully editable.
...
4. While the above run, **`components/ProgressExperience`** shows a 4-stage animated loading screen (dreaming up characters → writing → painting → binding), with a live "N of 5 illustrations ready" detail once images actually start coming back — never a bare spinner, and never marked complete until the real response succeeds. If generation fails, **`components/ErrorPanel`** shows a friendly failure screen with "Try again" (resubmits the same input) and "Change my idea" (returns to the form, prefilled) instead of a generic error.
5. **Storybook reader** (`components/StorybookReader`) presents the cover and each page one at a time, with Previous/Next buttons, a page indicator, arrow-key navigation, swipe support on touch devices, a print stylesheet, and a per-image "Regenerate this illustration" control.
